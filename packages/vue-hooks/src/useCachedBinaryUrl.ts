...
export const useCachedBinaryUrl = (binaryUrl: string | undefined) => {
  const cachedUrl = ref<string | undefined>(undefined);

  const updateCachedUrl = () => {
    if (!binaryUrl) {
    ...
    const existingCachedUrl = urls.get(binaryResourceUrl);
    if (existingCachedUrl) {
      const searchParams = new URLSearchParams(new URL(existingCachedUrl).search);
      ...
      if (expires && parseInt(expires, 10) * 1000 - 5_000 > Date.now()) {
        cachedUrl.value = existingCachedUrl;
        return;
      }
    }
  ...
  watch(() => binaryUrl, updateCachedUrl, { immediate: true });

  return computed(() => cachedUrl.value);
};